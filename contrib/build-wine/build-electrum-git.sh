...
ELECTRUM_GIT_URL=git://github.com/BTCP-community/electrum-zcl.git
BRANCH=master
...
cp -r electrum-git $WINEPREFIX/drive_c/electrum-zcl
...
#cp "$WINEPREFIX/drive_c/Program Files (x86)/ZBar/bin/"*.dll "$WINEPREFIX/drive_c/electrum-zcl/"
...
wine "$WINEPREFIX/drive_c/Program Files (x86)/NSIS/makensis.exe" electrum-zcl.nsi
#wine $WINEPREFIX/drive_c/Program\ Files\ \(x86\)/NSIS/makensis.exe electrum-zcl.nsis

DATE=`date +"%Y%m%d"`
cd dist
mv electrum-zcl.exe $NAME_ROOT-$DATE-$COMMIT_HASH.exe
mv electrum-zcl $NAME_ROOT-$DATE-$COMMIT_HASH
mv electrum-zcl-setup.exe $NAME_ROOT-$DATE-$COMMIT_HASH-setup.exe
