...
SYSTEM_CONFIG_PATH = "/etc/electrum-zcl.conf"
...
class SimpleConfig(object):
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "electrum-zcl.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-zcl.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-zcl.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-zcl.conf into user_config[]."""
    if not path:
        return {}
